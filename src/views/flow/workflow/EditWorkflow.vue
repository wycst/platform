...
<Card ref="mainCard">
	...
	编辑工作流
     ...
     <p slot="extra">
	      ...
	      <Button type="ghost" icon="trash-a" @click="clear">清除</Button>
	      <Button type="ghost" icon="android-arrow-back" @click="back">返回</Button>
     </p>   

     <Layout :style="{minHeight: '100vh'}" id="flowdesign">
	    ...
	    <Content :style="{padding: '0 0px 0px'}">
		 <Card>  
		    <Tabs ref="tabs" :animated="false" size="small" type="card" :style="{background : 'white'}" @on-click="clickTab">
...
<script>
	...
	mounted () {
	   ...
	   let id = this.$route.query.id;
           // 查询流程图数据
           this.$store.commit("loadProcessDesignData",{
	       id : id,
	       callback : (data)=> {
	           if(data.openStructuredData) {
		        this.designMode = 'open';
			this.$refs.bpmnRef.flowDesign.setDesignMode('open');
		   }
                   let structuredData = JSON.parse(data.openStructuredData || data.structuredData);
	           this.$refs.bpmnRef.flowDesign.setProcessData(structuredData,true);
	       }
	   });
	},
	methods : {
	   ...
	   save () {
	      ...
	      if(this.designMode == 'open') {
	          data.openStructuredData = structuredData;
	      } else {
